...
function TutorDetailsPage() {
  ...
  useEffect(() => {
    ...
    axiosInstance
      .get(`/get-prod/${tutorId}`)
      .then((res) => {
        setTutor(res.data || {}); // Provide a default value if the API response is empty
      })
      .catch((err) => {
        console.log(err);
      });
      ...
      <Section className={"relative"}>
        <div className="flex tablet-lg:flex-col-reverse justify-between items-stretch h-full  bg-opacity-20 my-10 rounded-3xl overflow-hidden shadow-2xl">
          <div className="basis-1/2 bg-primary dark:bg-dark-primary bg-opacity-10 dark:bg-opacity-5 backdrop-blur-sm flex justify-center items-center py-10">
            <img
              className="w-10/12 transform scale-x-[-1]"
              src="/Vectors/Details.svg"
              alt=""
            />
          </div>
          <div className="basis-1/2 bg-accent dark:bg-dark-accent bg-opacity-10 dark:bg-opacity-5 backdrop-blur-sm flex justify-center items-center py-4 px-4">
